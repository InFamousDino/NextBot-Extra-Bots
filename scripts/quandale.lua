------- SETTINGS -------
local tex = LoadSprite("MOD/imgs/quandale.png")
...
function init()
    ...
    if(HasKey("savegame.mod.nextbot_extra.speed")) then
        speedMove = GetFloat("savegame.mod.nextbot_extra.speed")
    end
    if(HasKey("savegame.mod.nextbot_extra.speed")) then
...
function tick(dt)
    ...
    local sound = LoadSound('MOD/sounds/messagequandale.ogg')
    ...
    if GetTime() > timePassed + sleep and useMusic then
        PlaySound(sound)
